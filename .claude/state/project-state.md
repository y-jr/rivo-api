...
| `fiscal` | ⚠ **Fatia mínima** (ADR-036), mais o motor de IRT/INSS (2026-08-30), os limiares de isenção de subsídios (2026-08-31) e a **exportação SAF-T (2026-09-07)**. O ficheiro sai válido contra o XSD oficial, verificado nos testes e contra a API a correr — mas **sem validade legal**: `SoftwareValidationNumber` a `"0"`, porque não há certificação da AGT. `Header`, a **tabela de clientes** (2026-09-08, ADR-059) — 100 clientes exportados e validados contra o XSD como o servidor os entrega — e a **tabela de impostos** (2026-09-08), que é de `fiscal` e não precisa de porta. Faltam fornecedores, produtos e o plano de contas; o XSD marca-os como opcionais, e é isso que permite crescer secção a secção sem nunca gerar ficheiro inválido. ⚠ **Os fornecedores estão bloqueados por falta de morada:** `Supplier.BillingAddress` é obrigatório no XSD e o agregado não tem morada nenhuma — decisão em aberto, ver `state/decisoes-em-aberto` se existir ou o relatório de 2026-09-08. Taxa plana com vigência e determinação (IVA, INSS); `IncomeTaxSchedule` — tabela de escalões progressivos — para o IRT; `SubsidyExemptionSchedule` — limiar em Kwanzas, mesmo padrão de vigência — para Alimentação e Transporte. Continua a não ser o motor fiscal completo: sem SAF-T, sem declarações periódicas |
