...
| Área | Estado |
|---|---|
| Código | 15 módulos (`Rivo.Messaging` novo, ADR-045) + 4 camadas de composição (`Rivo.Settings` ADR-041, `Rivo.EmployeePortal` ADR-042, `Rivo.Dashboard`, `Rivo.CustomerPortal` ADR-043), 85 projectos em `src/` |
| Superfície HTTP | 231 endpoints em 19 grupos de rota, mais `/health` — `POST /commercial/customers/{id}/owner`, `GET/POST /customer-portal/me/messages` e `GET /messaging/conversations` + `.../messages`/`.../closure` são os mais recentes (ADR-045) |
| ADRs | 45, aceites — ADR-045 (2026-09-04): mensagens directas com a equipa comercial, assíncronas, vendedor responsável só para notificar |
