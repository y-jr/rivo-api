...
| `fiscal` | ⚠ **Fatia mínima** (ADR-036), mais o motor de IRT/INSS (2026-08-30), os limiares de isenção de subsídios (2026-08-31) e a **exportação SAF-T (2026-09-07)**. O ficheiro sai válido contra o XSD oficial, verificado nos testes e contra a API a correr — mas **sem validade legal**: `SoftwareValidationNumber` a `"0"`, porque não há certificação da AGT. `Header`, a **tabela de clientes** (2026-09-08, ADR-059) — 100 clientes exportados e validados contra o XSD como o servidor os entrega — e a **tabela de impostos** (2026-09-08), que é de `fiscal` e não precisa de porta. e a **tabela de fornecedores** (2026-09-08), que obrigou a dar morada de facturação ao agregado `Supplier` — o XSD exige-a sem `minOccurs="0"`, e o Fornecedor não tinha morada nenhuma. e a **tabela de produtos** (2026-09-08), que trouxe `IInventoryCatalogue` — o primeiro contrato de leitura de `inventory`. Verificado contra a API a correr: 100 clientes, 30 fornecedores, 4 artigos, 35 entradas de imposto, válido contra o XSD como o servidor o entrega. **Falta só `GeneralLedgerAccounts`** no `MasterFiles` — é de `finance` e depende do PGC angolano, que o ADR-037 recusou inventar. Depois disso, `GeneralLedgerEntries` e `SourceDocuments`; o XSD marca-os como opcionais, e é isso que permite crescer secção a secção sem nunca gerar ficheiro inválido. Taxa plana com vigência e determinação (IVA, INSS); `IncomeTaxSchedule` — tabela de escalões progressivos — para o IRT; `SubsidyExemptionSchedule` — limiar em Kwanzas, mesmo padrão de vigência — para Alimentação e Transporte. Continua a não ser o motor fiscal completo: sem SAF-T, sem declarações periódicas |
...
| `Rivo.EmployeePortal` | "Próprio" resolvido por vínculo Identity → Employee (ADR-042). **Completo a 2026-09-07** — perfil, assiduidade, férias, documentos e recibos, **5 endpoints**. Duas dependências: `hr` publica `IEmployeeSelfService` (assiduidade, férias, documentos) e `payroll` publica `IPayrollSelfService` (recibos), ambos separados dos contratos existentes de propósito — `IEmployeeDirectory` responde "quem é esta pessoa" e é consumido por sete módulos, e dar-lhe o histórico de faltas daria a todos acesso a ele. ⚠ **Os recibos são só de folhas aprovadas**, e o filtro vive no armazenamento para nenhum consumidor futuro o poder esquecer. `verify-employee-portal.ps1` 8 casos |
...
| ADRs | **60**, todos aceites. O **ADR-060** (2026-09-08) fechou o K7 para a factura de venda: a cadeia `Hash` vive na `DocumentSeries`, ao lado do contador de numeração, porque a série já é o ponto onde a concorrência se resolve — era o sinal de alerta que o próprio ADR-036 tinha deixado. ⚠ **Não é a assinatura da AGT:** `HashControl` continua a `"0"` e uma factura do Rivo continua a não ser documento fiscal válido. O que a cadeia dá é detecção de adulteração. As facturas anteriores ficam sem elo, de propósito. A travessia que a verifica — `GET /finance/sales-invoices/chain` — entrou no mesmo dia, porque uma cadeia que ninguém percorre não detecta nada: só a travessia apanha um documento **removido**, já que os que sobram continuam consistentes consigo próprios. Provado contra a base de dados a correr, adulterando uma factura por SQL. A **linha** ganhou `ProductCode` e `UnitOfMeasure` (2026-09-08), obrigatorios e irreconstruiveis: o SAF-T exige-os em cada linha, e a linha so tinha descricao em texto livre. Texto livre e nao SKU de `inventory` -- uma linha de servico nao tem artigo em stock. Com isso, **`SourceDocuments`/`SalesInvoices` passou a sair** (2026-09-08), com facturas, **notas de credito** (que debitam em vez de creditar -- omiti-las sobredeclarava a receita) e **recibos** em `Payments`, e as **facturas de compra** em `PurchaseInvoices` (2026-09-08). ⚠ As compras anuladas ficam **de fora**: a seccao nao tem `DocumentStatus`, e nao ha onde escrever "A" -- assimetria do formato, nao decisao nossa. ⚠ O lado do movimento dos recibos (`CreditAmount`) e escolha por verificar profissionalmente: o XSD nao a fixa, validado contra o XSD como o servidor o entrega. ⚠ **Uma factura emitida com um codigo de imposto que o SAF-T nao aceita nao e exportavel, e nao ha correccao possivel** — a linha congela o codigo na emissao. Afecta os documentos anteriores a 2026-09-08 na base de dados de desenvolvimento. O **ADR-059** (2026-09-08) fixou como `fiscal` lê os módulos transaccionais para relatar: por porta declarada nas suas próprias palavras (`ISaftMasterData`) e ligada no composition root, e não por referência de projecto. A direcção é a que `docs/` manda; o que não podia ser é materializá-la como referência, porque `commercial` vai depender de `fiscal` para o imposto da venda e o ciclo é recusado por `Modules_HaveNoDependencyCycles`. Mesma resolução do ADR-034, terceira vez. O **ADR-058** (2026-09-07) pôs a identidade da empresa em configuração e não em entidade — o SAF-T foi o primeiro a precisar dela, e não existia em lado nenhum. É consequência do ADR-003: numa instalação sem multi-tenancy, a empresa **é** o deployment. Antes dele, uma linha só, do 050 ao 057: **ADR-050 (quem decide uma aprovação vem do token, não do corpo — corrige falha de segurança)**, **ADR-051 (ligar uma conta a um Colaborador já admitido, com permissão própria fora do perfil HR)**, **ADR-052 (desligar, e as decisões já tomadas continuam válidas)**, **ADR-053 (histórico do vínculo, sem tocar no caminho que decide)**, **ADR-054 (a admissão deixa de criar vínculos)**, **ADR-055 (o mesmo tratamento ao vínculo conta↔cliente)** e **ADR-057 (nenhum acto declara quem o pratica — sete rotas, incluindo a execução de pagamento)**. O 050 tornou o vínculo conta↔colaborador o único determinante de quem decide, o 051 deu-lhe a rota que faltava, o 052 deu-lhe a saída, o 053 deu-lhe memória, o 054 deixou-lhe um único caminho de entrada, o 055 levou o desenho ao `commercial`, e o 057 procurou a mesma forma no resto do sistema e encontrou-a sete vezes. Cada um foi aberto pelo anterior. (**ADR-056**, trilha independente da cultura, é de outra família.) |
...
- ~~`modules/messaging.md`.~~ **Escrito a 2026-09-04.** Os quinze módulos
  ...
  ⚠ **Este ficheiro afirmou o contrário durante dois dias**, em dois sítios,
  a par desta linha que já dizia a verdade. A tabela de Módulos e a de
  próximos passos continuaram a pedir o que estava feito, e uma revisão
  completa do documento a 2026-09-06 não apanhou a contradição — só
  apanhou quando alguém foi escrever o ficheiro e o encontrou lá.

  **Lição para este documento, e não para o `messaging`:** riscar um item
...
| **Depende só de código** | Utilizador de base de dados restrito em vez de `sa`. Observabilidade. Cobertura de Application nos seis módulos sem ela. Activos Fixos em `finance`, desbloqueados pelo ADR-039 e por escrever. Funil comercial (lead, oportunidade, proposta), fora de âmbito desde o ADR-036. **Ligar `procurement` a `inventory`** — ver "O que não existe"; precisa antes de uma decisão de negócio sobre que artigo corresponde a que linha de ordem |
