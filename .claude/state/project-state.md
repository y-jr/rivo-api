...
| `fiscal` | ⚠ **Fatia mínima** (ADR-036), mais o motor de IRT/INSS (2026-08-30), os limiares de isenção de subsídios (2026-08-31) e a **exportação SAF-T (2026-09-07)**. O ficheiro sai válido contra o XSD oficial, verificado nos testes e contra a API a correr — mas **sem validade legal**: `SoftwareValidationNumber` a `"0"`, porque não há certificação da AGT. `Header` e a **tabela de clientes** (2026-09-08, ADR-059) — 100 clientes exportados e validados contra o XSD como o servidor os entrega. Faltam fornecedores, produtos, plano de contas e tabela de taxas; o XSD marca-os como opcionais, e é isso que permite crescer secção a secção sem nunca gerar ficheiro inválido. Taxa plana com vigência e determinação (IVA, INSS); `IncomeTaxSchedule` — tabela de escalões progressivos — para o IRT; `SubsidyExemptionSchedule` — limiar em Kwanzas, mesmo padrão de vigência — para Alimentação e Transporte. Continua a não ser o motor fiscal completo: sem SAF-T, sem declarações periódicas |
