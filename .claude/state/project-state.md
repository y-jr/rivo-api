...
| Módulo | Estado |
|---|---|
| `identity` | Completo. JWT com sessão revogável, RBAC com 9 perfis (os 7 de negócio + `Cliente`, ADR-043, + `SuperAdmin`, ADR-058 — este último nunca atribuível em runtime, só por bootstrap), entrar com Google, bootstrap por seed, **gestão de conta** — mudar e repor password, activar/desactivar contas, ver e terminar sessões, retirar perfis |
...
| `hr` | Completo. Colaborador, Departamento, Cargo, Contrato, Assiduidade, Férias, Benefícios, Recrutamento, Onboarding/Offboarding. **Correcções desde 2026-09-16** (ADR-063): nome do colaborador, transferência de departamento, nome/responsável do departamento, nome/nível do cargo — os primeiros `PUT` do projecto. A marca de autoridade de aprovação **não** é editável (BR-20), e o estado do colaborador continua a seguir a cessação do contrato |
...
| `Rivo.EmployeePortal` | "Próprio" resolvido por vínculo Identity → Employee (ADR-042). **Perfil, assiduidade, férias, documentos e recibos** (ADR-062, 2026-09-16) — 5 endpoints, nenhum com `employeeId`, nenhuma permissão nova: compõe `hr` e `payroll` só por contrato publicado. Só folhas **aprovadas** dão recibo, filtro na consulta. ⚠ Leitura apenas — **escrever** pelo portal (pedir férias, justificar falta) continua por fazer. `verify-employee-portal.ps1` 19 casos |
...
| Superfície HTTP | **250 endpoints** em **20 grupos de rota** (2026-09-12: +1, `POST /hr/employees/{id}/positions/direct`, ADR-058). Os maiores: `hr` 41, `finance/ledger` 30, `procurement` 19, `inventory` 19, `finance` 18, `payables` 16, `fleet` 16, `identity` 14, `projects` 13 |
...
| Frontend | ⚠ **Contagem desactualizada — precisa de recontagem.** A linha "13 dos 15, 51 ficheiros, 28 rotas, 132 chamadas" é de 2026-09-06. Entre essa data e 2026-09-12 ganharam ecrã: Portal do Cliente, Folha Salarial (`payroll`), Férias/Benefícios/Recrutamento/Onboarding/Offboarding (`hr`), Documentos (`documents`) e Impostos (`fiscal`) — pelo menos 9 rotas novas em `ROTAS_REAIS`. Continuam de fora só `notifications` (sem ecrã dedicado — o sino é do layout) e `messaging` (portal do cliente, sem lado interno). Vive em `../front`, repositório próprio **sem remoto configurado** |
| Perfis de Acesso | **9** — os 7 do documento de produto, mais `Cliente` (ADR-043) e `SuperAdmin` (ADR-058, 2026-09-12). `Admin` tem **71 permissões**, confirmado em base a 2026-09-05 (a 71.ª é `hr.employees.link_account`, ADR-051). `SuperAdmin` tem essas 71 mais `hr.positions.assign_direct`, que nenhum outro perfil tem — e, ao contrário dos outros oito, **nunca aparece nem é atribuível** por `GET/POST /identity/*/roles`, só nasce por `Bootstrap:Users`. A autorização dos portais continua por vínculo de identidade, não por permissão — as excepções são `documents.write` (comprovativo de pagamento, ADR-044) e as permissões próprias de `Dashboard` e `Analytics`, concedidas a `Manager` porque o documento de produto o nomeia e ele não tem as permissões dos módulos subjacentes |
