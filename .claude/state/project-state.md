...
| Testes automatizados | **1 231** em **32 projectos**, todos passam. Por camada: **830 de domínio**, **367 de Application**, 21 de arquitectura, 9 de API, 4 de integração (Testcontainers). A distribuição por módulo é que é desigual — ver "O que não existe" |
...
- **Cobertura de Application em nove dos quinze módulos.** Só `finance`
  ...
  `inventory` (16), `fleet` (15), `commercial` (11) e `approval` (8) a têm. Os
...
4. **Cobertura de Application nos outros módulos** — `finance` tem 162 testes
   ...
   `inventory` 16, `fleet` 15, `commercial` 11 e `approval` 8; os outros seis
   módulos, nenhum.
