...
| `fiscal` | ⚠ **Fatia mínima** (ADR-036), mais o motor de IRT/INSS (2026-08-30), os limiares de isenção de subsídios (2026-08-31) e a **exportação SAF-T (2026-09-07)**. O ficheiro sai válido contra o XSD oficial, verificado nos testes e contra a API a correr — mas **sem validade legal**: `SoftwareValidationNumber` a `"0"`, porque não há certificação da AGT. Só `Header` e `MasterFiles` vazio por agora; o XSD marca tudo o resto como opcional, e é isso que permite crescer secção a secção sem nunca gerar ficheiro inválido. Taxa plana com vigência e determinação (IVA, INSS); `IncomeTaxSchedule` — tabela de escalões progressivos — para o IRT; `SubsidyExemptionSchedule` — limiar em Kwanzas, mesmo padrão de vigência — para Alimentação e Transporte. Continua a não ser o motor fiscal completo: sem SAF-T, sem declarações periódicas |
